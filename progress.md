...
| M8 RL (MuJoCo CPU, ONNX export) | [x] | **Done 2026-08-22**: `h1_rl_policy` pkg — numpy population-search policy on planar-biped MuJoCo proxy (OBS 12/ACT 4), ONNX export via hand-built graph (checker OK, matches numpy fwd to 9e-9), trained best_return 281.98, int8 quantize hook (M9) verified with onnxruntime. Tests 9 (suite total 415). See TASK-h1_rl_m8.md. **Skill expansion same day** (TASK-h1_rl_skills.md): fixed 3 silent physics/training bugs (MuJoCo degree-vs-radian default made all joints ±1° statues; actuator/action order mismatch; trainer passed scalar actions) → retrained stand honestly (211), added single-leg squats L+R (max flexion 1.47/1.49 rad, curriculum-trained) and backflip (full −2π rotation, landed=True; reduced-order pitch-motor template). 4 ONNX policies exported. Package tests 19; suite total **425/425 across 9 pkgs**. |
