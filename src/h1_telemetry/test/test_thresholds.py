...
def test_no_breaches():
    ...
    sample = {'joint_states_hz_min': 50.0, 'odometry_hz_min': 30.0,
              'cpu_load_max': 0.5, 'ram_used_mb_max': 800.0}
    assert ev.evaluate(sample) == []


def test_min_breach():
    ev = ThresholdEvaluator(THRESHOLDS)
    out = ev.evaluate({'joint_states_hz_min': 25.0, 'odometry_hz_min': 2.5})
    assert out == [Breach('joint_states_hz_min', 25.0, 30.0, BREACH_MIN)]


def test_max_breach():
    ev = ThresholdEvaluator(THRESHOLDS)
    out = ev.evaluate({'cpu_load_max': 0.99, 'ram_used_mb_max': 1600.0})
    assert out == [Breach('cpu_load_max', 0.99, 0.95, BREACH_MAX)]


def test_multiple_breaches_preserve_order():
    ev = ThresholdEvaluator(THRESHOLDS)
    sample = {'joint_states_hz_min': 10.0, 'odometry_hz_min': 1.0,
              'cpu_load_max': 1.0, 'ram_used_mb_max': 1800.0}
...
def test_boundary_values_not_breached():
    ...
    assert ev.evaluate({'joint_states_hz_min': 30.0}) == []
    assert ev.evaluate({'cpu_load_max': 0.95}) == []


def test_missing_and_unknown_keys_ignored():
    ev = ThresholdEvaluator(THRESHOLDS)
    assert ev.evaluate({}) == []
    assert ev.evaluate({'not_a_threshold': 999.0, 'foo_min': 1.0}) == []
...
def test_nan_and_inf_values():
    ev = ThresholdEvaluator(THRESHOLDS)
    assert ev.evaluate({'cpu_load_max': float('nan')}) == []  # nan > x is False
    assert ev.evaluate({'cpu_load_max': float('inf')}) != []
...
def test_from_yaml(tmp_path):
    ...
    assert ev.evaluate({'joint_states_hz_min': 1.0})[0].name == 'joint_states_hz_min'
    assert ev.evaluate({'cpu_load_max': 0.9}) == []
